...
def main():
    ...
    G = Generator(nz=nz+7, nch_g=nch_g).to(device)   # 入力ベクトルの次元は、nz+クラス数7
    G.apply(weights_init)

    D = Discriminator(nch=3+7, nch_d=nch_d).to(device)   # 入力Tensorのチャネル数は、nch+クラス数7
    D.apply(weights_init)

    criterion = nn.MSELoss()

    optimizerD = optim.Adam(D.parameters(), lr=lr, betas=(beta1, 0.999), weight_decay=1e-5)
    optimizerG = optim.Adam(G.parameters(), lr=lr, betas=(beta1, 0.999), weight_decay=1e-5)
    ...
    for epoch in range(n_epoch):
        ...
        # ---------  save model  ----------
        if (epoch + 1) % 10 == 0:  
            torch.save(G.state_dict(), '{}/G_epoch_{}.pth'.format(outf, epoch + 1))
            torch.save(D.state_dict(), '{}/D_epoch_{}.pth'.format(outf, epoch + 1))
